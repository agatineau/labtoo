...
function initSameHeight() {
    ...
    jQuery('#features .features-area').sameHeight({
...
StarRating.prototype = {
    ...
    refreshClasses: function () {
        ...
        if (this.hovering) {
            for (var j = 0; j <= this.hoverIndex; j++) {
                lib.addClass(this.items[j], this.options.activeClass);
            }
        } else {
            if (typeof this.settedIndex === 'number') {
                for (var j = 0; j <= this.settedIndex; j++) {
                    lib.addClass(this.items[j], this.options.settedClass);
                }
            } else {
