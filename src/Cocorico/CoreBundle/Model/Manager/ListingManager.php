...
class ListingManager extends BaseManager
{
    ...
    {
        /** @var ExperimentTranslation $experimentTranslation */
        foreach ($listing->getExperiment()->getTranslations() as $experimentTranslation) {
            /** @var ListingTranslation $listingTranslation */
            foreach ($listing->getTranslations() as $listingTranslation) {
                if ($listingTranslation->getLocale() != $experimentTranslation->getLocale()) continue;
                $listingTranslation->setTitle($experimentTranslation->getTitle());
                $listingTranslation->setKeywords($experimentTranslation->getKeywords());
                $listingTranslation->setDescription($experimentTranslation->getDescription());
            }
        }
        ...
        /** @var ExperimentTranslation $experimentTranslation */
        foreach ($listing->getExperiment()->getTranslations() as $experimentTranslation) {
            $locale = $experimentTranslation->getLocale();
            if (!$listing->getTranslations()->containsKey($locale)) {
                $listingTranslation = new ListingTranslation();
                $listingTranslation->setLocale($locale);
                $listingTranslation->setTitle($experimentTranslation->getTitle());
                $listingTranslation->setDescription($experimentTranslation->getDescription());
                $listingTranslation->setKeywords($experimentTranslation->getKeywords());
    ...
    /**
     * Create answers while listing deposit.
     *
     *
     * @param  Listing $listing
     * @param  array   $answers Id(s) of ListingAnswer(s) selected
     *
     * @return Listing
     */
    public function addAnswers(Listing $listing, array $answers)
    {
        foreach ($answers as $i => $answer) {

            $question = $this->em->getRepository('CocoricoExperimentBundle:Question')->find($answer['question']);

            $value = isset($answer['value']) ? $answer['value'] : '';
            $value = is_array($value) ? implode('|', $value) : $value;

            $listingAnswer = new ListingAnswer();
            $listingAnswer->setQuestion($question);
            $listingAnswer->setValue($value);
            $listing->addAnswer($listingAnswer);
        }
