...
class ListingSearchController extends Controller
{
    ...
    protected function getMarkers(Request $request, $results, $resultsIterator)
    {
        ...
        foreach ($results->getIterator() as $i => $result) {
            ...
            $image = $liipCacheManager->getBrowserPath($imagePath . $imageName, 'cocorico_search_result_image', array());
            ...
            $certifiedClass = $certifiedIcon = '';
            if($listing['certified'])
            {
                $certifiedClass = 'listing-certified';
                $certifiedIcon = '<div class="certified-icon"><img src="/images/certified-stars.svg" /></div><div class="certified-corner-icon"></div>';
            }
